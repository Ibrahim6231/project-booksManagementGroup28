...
const createBooks = async function (req, res) {     //1.authentication 2.bookValidation 3.createBooks
    try {
        ...
        let data= { title:title, excerpt:excerpt, userId:userId, ISBN:ISBN, category:category, subcategory:subcategory, releasedAt:releasedAt }

        const files = req.files;                                                    
        let uploadedFileURL;
        if(files && files.length > 0){
          uploadedFileURL = await uploadFile(files[0]);
        }else{return res.status(400).send({status:false, message:"no file found"})}
